#!/usr/bin/env python3
"""Backfill applied_address on applied jobs that are missing it.

Resolves from resume header city (resume.tex / resume.pdf) via the synthetic
apartment bank in scripts/fastfill/fixtures/us_apartment_addresses.json.
Falls back to job location when the resume header has no City, ST.

Usage:
  python3 scripts/backfill_applied_addresses.py [--dry-run] [--job-id ID]
"""
...
def _resolve_address(job: dict) -> str | None:
    ...
    try:
        ...
        print(f"warn: address resolve failed for {job.get('id')}: {exc}")
        return None
    return format_address_line(pick) or None


def backfill(dry_run: bool = False, job_ids: set[str] | None = None) -> dict:
    stats = {"checked": 0, "already_set": 0, "backfilled": 0, "skipped": 0}
    ...
    with locked_jobs_for_write() as data:
        ...
        for job in jobs:
            ...
            if not address:
                stats["skipped"] += 1
                print(f"skip: {jid} — no resume or could not resolve address")
                continue
            pending.append((jid, address))
            if not dry_run:
                ...
                stats["backfilled"] += 1
                print(f"would backfill: {jid} -> {address}")
